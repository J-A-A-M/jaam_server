...
import { api, type ClaimCodeResult, type JaamMap, type JaamMapInput } from "@/lib/api";
...
export function JaamMapModal({ open, onClose, editing, defaultChipId, onSuccess }: JaamMapModalProps) {
  ...
  // Одноразовий - показується лише в цій відповіді, ніде на сервері не зберігається (лише
  // SHA-256 хеш у Redis з TTL).
  ...
  // chip_id щойно створеного (в цій сесії модалки) або переданого через editing запису -
  // відрізняється від editing?.chip_id тим, що "Видати код активації" може сам створити запис
  // просто зі стану форми (без окремого натискання "Зберегти" спершу), і після цього функції
  // whitelist/save мають діяти як editing, хоча prop editing лишається тим, яким був при
  // відкритті модалки (керується батьком, не цим компонентом).
  const [persistedChipId, setPersistedChipId] = React.useState<string | null>(editing?.chip_id ?? null);
  ...
  React.useEffect(() => {
    ...
    setPersistedChipId(editing?.chip_id ?? null);
    setClaimed(null);
    setError("");
  ...
  const claimCodeMut = useMutation({
    mutationFn: async () => {
      // Форма ще ніде не збережена (створюємо "Нову JAAM-мапу" і одразу тиснемо на видачу
      // коду, без окремого "Зберегти") - створюємо запис зі стану форми тут-таки, щоб не
      // змушувати адміна зберігати двічі.
      if (!persistedChipId) {
        const chipId = form.chip_id.trim();
        if (!chipId) throw new Error("Спочатку вкажіть Chip ID");
        await api.createMap({ ...form, chip_id: chipId });
        setPersistedChipId(chipId);
        qc.invalidateQueries({ queryKey: ["inventory"] });
        qc.invalidateQueries({ queryKey: ["device"] });
        return api.issueClaimCode(chipId);
      }
      return api.issueClaimCode(persistedChipId);
    },
    onSuccess: (res) => {
      setClaimed(res);
      setWhitelisted(true);
  ...
  const whitelistMut = useMutation({
    mutationFn: (next: boolean) => api.setDeviceWhitelisted(persistedChipId!, next),
    onSuccess: (res) => {
  ...
  const saveMut = useMutation({
    mutationFn: (body: JaamMapInput) =>
      persistedChipId ? api.updateMap(persistedChipId, body) : api.createMap(body),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["inventory"] });
      qc.invalidateQueries({ queryKey: ["device"] });
      onClose();
  ...
  return (
    <Modal open={open} onClose={onClose} title={persistedChipId ? `Редагувати ${persistedChipId}` : "Нова JAAM-мапа"}>
      <form onSubmit={handleSubmit} className="space-y-3">
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="mb-1 block text-xs text-muted-foreground">Chip ID *</label>
            <Input value={form.chip_id} onChange={(e) => setForm({ ...form, chip_id: e.target.value })} disabled={!!persistedChipId} required className="font-mono" placeholder="напр. a1b2c3d4e5f6" />
          </div>
          <div>
        ...
        {form.chip_id.trim() !== "" && user?.role === "admin" && (
