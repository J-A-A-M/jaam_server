...
## Обласні центри (кружечки міст)

`tools/add_cities.py` читає `ukraine_lowpoly.svg`, дописує шар `<g id="cities">`
і пише `ukraine_lowpoly_with_cities.svg` (окремий файл, `ukraine_lowpoly.svg` не чіпає).

### Чому не всі 25 — кружечки
- **Київ** (`raion-14`), **Харків** (`raion-1293`), **Запоріжжя** (`raion-564`)
  вже мають власний raion-полігон у базовій мапі — для них у `#cities`
  дублюється цей полігон як є (`data-id` = regionId; для Києва `raion-14`
  формально належить Київській області, але геометрично і є містом —
  перевірено: спроєктована точка Києва потрапляє точно в нього, площа на
  порядок менша за сусідні raion-и Київщини).
- **Решта 22** обласних центри не мають окремого полігону в джерельних
  даних → намальовані колом.

### Проєкція lon/lat → viewBox
Афінна (least squares) з 24 калібрувальних пар: центроїд `oblast-{id}`
полігону з `ukraine_lowpoly.svg` ↔ `"location"` відповідної `*-область` записи
з `../regions.json` (це геометричний центр області, не міста — тому й лишається
похибка кілька viewBox-одиниць, компенсується кроком нижче).

### Радіус і позиція кола
1. Реальні lat/lon адмінцентру (`*-CITY` запис у `regions.json`) → проєкція.
2. Якщо проєктована точка впала поза "своєю" областю (похибка проєкції на
   краю мапи) — снепається на найближчу точку всередині полігону.
3. `R = 2.3` viewBox-одиниці (цільовий діаметр — приблизно як фактичний
   "відбиток" Харкова/Запоріжжя в цьому масштабі) — `geom.buffer(-R)`;
   якщо не порожньо, коло радіуса R ставиться максимально близько до міської
   точки, лишаючись повністю всередині своєї області (кордон не перетинає).
4. Якщо область вужча за 2R (наприклад витягнута/маленька) — бінарний пошук
   максимального радіуса, що ще влазить.
5. Перевірка кола-на-кола (`d < r1+r2`) між усіма парами міст — перетину
   бути не повинно (border-containment це гарантує, бо області не
   перекриваються, але скрипт все одно падає з `SystemExit`, якщо знайде).

### `data-id`
`regionId` з відповідного `*-CITY` запису `regions.json` (не внутрішній ключ
типу `"KHARKIV-CITY"`, не raion id з мапи) — узгоджено з тим, як WS-сервер/API
адресують міста.

### Структура
```
<g id="cities">
  <g class="city" data-oblast="{oblastId}" data-id="{regionId}"><title>{назва}</title>
    <circle cx cy r/>          <!-- 22 обласних центри -->
    | <polygon points/>        <!-- Київ -->
  </g> ...
```
`fill:none` — контур без заливки.

### Регенерація
```
.venv/bin/python design/tools/add_cities.py   # читає ukraine_lowpoly.svg, пише *_with_cities.svg
```
Дані міст (regionId/назва/lat/lon) і калібрувальні пари захардкоджені у
константах `CITIES`/`CALIB` вгорі скрипта — новий обласний центр (якщо
адмінподіл зміниться) додається туди вручну.
